...
async function getCachedPrice(symbol, force = false) {
  const cached = priceCache.get(symbol);
  const ttl = isMutualFund(symbol) ? 3600000 : 30000;
  // force=true のときはキャッシュをスキップ（投信は除く）
...
app.get('/api/prices', async (req, res) => {
  ...
  const results = await Promise.allSettled(symbols.map(s => getCachedPrice(s, force)));
